...
const Cat = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("catStatus")) {
      setStatusChange(localStorage.getItem("catStatus"));
    } else {
      setStatusChange(418);
      localStorage.setItem("catStatus", status);
    }
  });
  ...
  useEffect(() => {
    ...
    const resetStatus = () => {
      setStatusChange(418);
      localStorage.setItem("catStatus", 418);
    };

    const resetInactivityTimer = () => {
      clearTimeout(inactivityTimer);
      inactivityTimer = setTimeout(resetStatus, 10 * 60 * 1000); // 10 minutes
    };
